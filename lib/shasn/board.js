...
export function gerrymander(
  ...
  from,
  to,
  { allowMajority = false, blockVolatileDestination = false } = {}
) {
  ...
  // Submerged (p.17): "These voters cannot be placed into Volatile Areas."
  // Ordinary gerrymandering may move INTO a Volatile Area — that is how you
  // deliberately trigger a Headline — so this is off unless a card asks for it.
  if (blockVolatileDestination && isVolatile(to.zoneId, to.areaIndex)) {
    return { error: 'These voters cannot be placed into Volatile Areas' }
  }
...
/**
 * Total voters a player has on the board, majority or not. Used as a
 * tie-breaker and to show effort that has not yet converted into points.
 */
export function totalVoters(board, playerId) {
  return ZONE_IDS.reduce((n, z) => n + voterCount(board, z, playerId), 0)
}

/**
 * Zones this player leads but has not yet won, with how far short they are.
 *
 * Scoring is all-or-nothing per zone: you get the majority requirement or you
 * get nothing. That means a player can be doing well for a dozen turns while
 * still reading zero. This surfaces the work in progress.
 */
export function contestedZones(board, playerId) {
  const out = []
  for (const zoneId of ZONE_IDS) {
    if (majorityHolder(board, zoneId)) continue
    if (isZoneSettled(board, zoneId)) continue

    const counts = voterCounts(board, zoneId)
    const mine = counts[playerId] || 0
    if (mine === 0) continue
    ...
    out.push({
      ...
      needed: req - mine,
      majority: req,
      leading: mine === best && Object.values(counts).filter((v) => v === best).length === 1,
      reachable: mine + emptyAreas(board, zoneId) >= req,
    })
  }
  return out
}
...
/**
 * Best score still achievable: what is banked, plus every zone the player could
 * still reach the requirement in.
 */
export function projectedScore(board, playerId) {
  let n = scores(board)[playerId] || 0
  for (const c of contestedZones(board, playerId)) {
    if (c.reachable) n += c.majority
  }
  return n
}
 ...
 *
 * p.19 scores majority voters only — "not the number of zones you capture". The
 * printed tie-breaker is the real-life privilege calculator, which is explicitly
 * non-binding and not something to automate, so ties fall through to board
 * position instead: zones held, then total voters placed. If players are still
 * level after that the game is a genuine draw and is reported as one rather than
 * inventing a winner.
 */
export function standings(board, players) {
  ...
  const rows = players.map((p) => ({
    ...
    voters: totalVoters(board, p.id),
    contested: contestedZones(board, p.id),
    projected: projectedScore(board, p.id),
  }))

  rows.sort(
    (a, b) =>
      b.score - a.score ||
      b.zonesHeld.length - a.zonesHeld.length ||
      b.voters - a.voters
  )

  // Dense ranking, so genuine ties share a rank.
  let rank = 0
  rows.forEach((row, i) => {
    const prev = rows[i - 1]
    const same =
      ...
      prev.zonesHeld.length === row.zonesHeld.length &&
      prev.voters === row.voters
    if (!same) rank = i + 1
    row.rank = rank
  })
  rows.forEach((row) => {
...
/** Per-zone scorecard: who holds what and what it was worth. */
export function scoreBreakdown(board, players) {
  const nameOf = (id) => players.find((p) => p.id === id)?.nickname ?? null
  return ZONE_IDS.map((zoneId) => {
    ...
    return {
      ...
      holder,
      holderName: holder ? nameOf(holder) : null,
      points: holder ? ZONES[zoneId].majority : 0,
