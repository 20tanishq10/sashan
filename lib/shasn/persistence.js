...
export function viewFor(game, viewerPlayerId) {
  ...
  return {
    ...
    // The shot clock is public — everyone watches the same countdown, and any
    // player may fire it once it expires.
    ideologyDeadline: game.ideologyDeadline ?? null,
    // Haggling is public at a table, so open offers are visible to everyone —
    // but the specific Conspiracy Cards inside one are not, unless you are a
    // party to it. Otherwise an offer would leak the proposer's hand.
    pendingTrades: (game.pendingTrades || [])
      .filter((t) => t.status === 'pending')
      .map((t) => {
        const party = viewerPlayerId === t.proposerId || viewerPlayerId === t.targetId
        return {
          ...t,
          offer: {
            ...t.offer,
            conspiracyCards: party ? t.offer.conspiracyCards : [],
            conspiracyCardCount: t.offer.conspiracyCards.length,
          },
        }
      }),
    ...
    // Decks are hidden; only their sizes are public. Draw and discard counts are
    // both public on the physical board — you can see how tall each pile is.
    ideologyDeck: deckCounts(game.ideologyDeck),
    conspiracyDeck: deckCounts(game.conspiracyDeck),
    headlineDeck: deckCounts(game.headlineDeck),
...
function deckCounts(d) {
  return {
    drawCount: d?.drawPile?.length || 0,
    discardCount: d?.discard?.length || 0,
    size: (d?.drawPile?.length || 0) + (d?.discard?.length || 0),
  }
}
