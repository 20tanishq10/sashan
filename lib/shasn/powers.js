...
/**
 * Whether Polo Retreat is currently lending this player that Ideologue's L3.
 *
 * Kept next to claimUse because it is the same question — "may you use this
 * power" — just answered from a card rather than from your own cards.
 */
export function borrowsLevel3(game, player, ideologueId) {
  return Effects.canBorrowLevel3(
    player,
    game.players,
    (p) => Ideology.unlockedPowers(p.ideologyCards),
    ideologueId
  )
}
...
function claimUse(game, ideologueId, level) {
  ...
  // Polo Retreat lends a Level 3 power you have not unlocked. It is an extra
  // route to the power, not extra uses of one you already have, so it is only
  // consulted when your own cards do not already grant it.
  const borrowed =
    level === 3 &&
    !Ideology.unlockedPowers(player.ideologyCards)[ideologueId]?.level3 &&
    borrowsLevel3(game, player, ideologueId)

  if (borrowed) {
    const def = IDEOLOGUES[ideologueId].level3
    const used = player.powerUses[`${ideologueId}_l3`] || 0
    if (used >= def.usesPerTurn) {
      return { error: `${def.name} is exhausted this turn` }
    }
    return {
      player,
      borrowed: true,
      commit: (g) =>
        setPlayer(g, player.id, {
          powerUses: Ideology.recordPowerUse(
            g.players.find((p) => p.id === player.id).powerUses,
            ideologueId,
            level
          ),
        }),
...
export function availableActions(game) {
  ...
  return defs
    .map(([ideologue, level, action]) => {
      ...
      const own = Ideology.unlockedPowers(player.ideologyCards)[ideologue][`level${level}`]

      // Polo Retreat lends a Level 3 you have not unlocked. It has to appear in
      // this list or the player has a power the UI never offers them.
      const borrowed = !own && level === 3 && borrowsLevel3(game, player, ideologue)
      const used = player.powerUses[`${ideologue}_l${level}`] || 0
      ...
      return {
        ...
        name: def.name,
        text: def.text,
        unlocked: own || borrowed,
        borrowed: Boolean(borrowed),
        remaining: borrowed ? Math.max(0, def.usesPerTurn - used) : remaining,
      }
    })
    .filter((p) => p.unlocked)
}
