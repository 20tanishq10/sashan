...
import { IDEOLOGUE_IDS as ALL_IDEOLOGUES } from './constants'
...
/**
 * Polo Retreat (p.17) — "Choose 2 players. They can use each other's unlocked
 * Level 3 Ideologue Powers in addition to their own in their next turns."
 *
 * Returns the Ideologue ids this player may use at Level 3 on top of whatever
 * they have unlocked themselves. Empty for everyone not on the card.
 *
 * `sharedLevel3With` is either a player id — use whatever THEY have unlocked —
 * or the string 'any', which is the card's own clarification: "If either of the
 * players don't have any unlocked Level 3 powers, both players can use any one
 * Level 3 power of their choice." A pairing where one side has nothing to lend
 * would otherwise be a dead card.
 */
export function borrowedLevel3(player, players = [], unlockedFor) {
  const partner = effectsOf(player).sharedLevel3With
  if (!partner) return []
  if (partner === 'any') return ALL_IDEOLOGUES.slice()

  const lender = players.find((p) => p.id === partner)
  if (!lender) return []
  const unlocked = unlockedFor(lender)
  return ALL_IDEOLOGUES.filter((id) => unlocked[id]?.level3)
}

/** Whether this player may use that Ideologue's L3 power via Polo Retreat. */
export function canBorrowLevel3(player, players, unlockedFor, ideologueId) {
  return borrowedLevel3(player, players, unlockedFor).includes(ideologueId)
}
