...
export function majorityTrack(board, zoneId) {
  ...
  // The same information as `segments`, collapsed into runs. A zone can have 21
  // areas, and 21 separate ticks in a 90px plaque is a barcode rather than a
  // reading — so the plaque draws proportional blocks instead, one per holder.
  const runs = []
  for (const seg of segments) {
    const last = runs[runs.length - 1]
    if (last && last.owner === seg.owner) last.count += 1
    else runs.push({ owner: seg.owner, count: 1 })
  }
