...
export function createGame({
  ...
  const game = {
    ...
    diversions: [],
    interrupt: null,
    // A card that stops the game and walks it round the table, one seat at a
    // time — Submerged and A Trip To Goalpara. See lib/shasn/rounds.js.
    round: null,
    pendingTrades: [],
    tradeSeq: 0,
  ...
  let next = applySideEffects(
    {
      ...
      // Jumla is the one Conspiracy Card that does not go to the discard pile
      // when played: it moves into an Ideology stack and stays on the table
      // until somebody buys it. Discarding it would let it be drawn twice.
      conspiracyDeck:
        cardId === Jumla.JUMLA_ID
          ? game.conspiracyDeck
          : Deck.discardCard(game.conspiracyDeck, cardId),
    },
    result,
    cardId
  )
...
/**
 * Take Jumla off whoever is holding it.
 *
 * "Opponents can take this card by paying you as many resources as the Ideologue
 * level this card is on." The payment goes to the holder, not the Reserve.
 *
 * Deliberately not turn-gated. The card says "opponents", with no timing
 * restriction, and the interesting play is snatching it the moment it takes
 * somebody over a threshold — which is rarely on your own turn.
 */
export function takeJumla(game, { playerId, payment = null }) {
  const quote = Jumla.canTake(game, playerId, payment)
  if (quote.error) return { error: quote.error }

  const buyer = game.players.find((p) => p.id === playerId)
  const seller = game.players.find((p) => p.id === quote.from)
  const at = Jumla.findJumla(game)

  // Straight from buyer to seller. Chai-Paani diverts payments made INTO the
  // Reserve, and this is not one, so it is not routed through Effects.
  let next = updatePlayer(game, buyer.id, {
    pool: R.subtractPools(buyer.pool, quote.payment),
  })
  next = updatePlayer(next, seller.id, {
    pool: R.addPools(next.players.find((p) => p.id === seller.id).pool, quote.payment),
  })

  // It keeps the Ideologue it was under; the new holder may re-file it at the
  // end of their own turn.
  next = { ...next, players: Jumla.place(next.players, buyer.id, at.ideologue) }

  const lost = Ideology.unlockedPowers(seller.ideologyCards)
  const after = Ideology.unlockedPowers(next.players.find((p) => p.id === seller.id).ideologyCards)
  next = addLog(next, {
    type: 'conspiracy',
    message: `${buyer.name} took Jumla from ${seller.name} for ${quote.price} resource(s).`,
  })
  for (const id of Object.keys(lost)) {
    if (lost[id].level3 && !after[id].level3) {
      next = addLog(next, {
        type: 'power',
        message: `${seller.name} lost a Level 3 power with Jumla.`,
      })
    }
  }

  return { game: next, price: quote.price }
}

/**
 * Re-file Jumla under a different Ideologue.
 *
 * "At the end of your turn, you may place this card under a different
 * Ideologue" — and the clarification stresses it is optional, so this is an
 * action the holder takes rather than a prompt they must answer.
 */
export function moveJumla(game, { playerId, ideologue }) {
  const at = Jumla.findJumla(game)
  if (!at) return { error: 'Jumla is not in play' }
  if (at.playerId !== playerId) return { error: 'You are not holding Jumla' }
  if (activePlayer(game).id !== playerId) {
    return { error: 'Jumla can only be re-filed at the end of your own turn' }
  }
  if (game.turnPhase !== TURN_PHASES.ACTIONS) {
    return { error: 'Finish your Ideology Card first' }
  }
  if (ideologue === at.ideologue) return { error: 'Jumla is already under that Ideologue' }
  if (!IDEOLOGUE_IDS.includes(ideologue)) return { error: 'Unknown Ideologue' }

  const next = { ...game, players: Jumla.place(game.players, playerId, ideologue) }
  return {
    game: addLog(next, {
      type: 'conspiracy',
      message: `${activePlayer(game).name} moved Jumla under a different Ideologue.`,
    }),
  }
}

// ---------------------------------------------------------------------------
// Going round the table (p.17)
// ---------------------------------------------------------------------------

/**
 * Take your slot in an open round.
 *
 * Two cards work this way, and they share everything except what a slot does:
 *
 *   Submerged           every player Gerrymanders one voter, starting with the
 *                       player who triggered it
 *   A Trip To Goalpara  the next 3 players each cash out an open Voter Card
 *
 * `action` is 'act' or 'pass'. Passing is always allowed — a player with no
 * Gerrymandering Rights has nothing to do with their slot, and a round that
 * could not be passed would deadlock the game.
 */
export function actInRound(game, rng, { playerId, action = 'act', ...payload }) {
  const round = game.round
  if (!round) return { error: 'Nothing is going round the table' }
  if (!Rounds.isCurrent(round, playerId)) {
    const whose = game.players.find((p) => p.id === Rounds.currentPlayer(round))
    return { error: `It is ${whose?.name || 'someone else'}'s turn to act on this card` }
  }
  ...
  if (action === 'pass') {
    ...
    next = addLog(next, {
      type: 'resolution',
      message: `${player.name} passed on ${round.cardName}.`,
    })
  } else {
    const applied =
      round.kind === 'gerrymander'
        ? applyRoundGerrymander(next, round, player, payload)
        : round.kind === 'cashOutVoter'
          ? applyRoundCashOut(next, round, player, payload)
          : { error: `Unknown round: ${round.kind}` }
  ...
  next = { ...next, round: Rounds.advance(next.round, { playerId, action, note }) }

  // Everyone has had their slot. Take the round down and let the game resume.
  if (Rounds.isFinished(next.round)) next = closeRound(next, rng)

  next = announceMajorities(next, game.board)
  return { game: checkGameEnd(next) }
}

/** One player's Gerrymander during Submerged. */
function applyRoundGerrymander(game, round, player, { rightsZoneId, from, to }) {
  if (!rightsZoneId || !from || !to) return { error: 'Choose a voter to move and where to put it' }

  // Regular gerrymandering rules still apply (the card's own clarification), so
  // this goes through the same board function as an ordinary move. What the
  // card changes is passed as options: majority voters may be moved, and the
  // destination may not be a Volatile Area.
  //
  // Notably this does NOT consume the player's per-turn Gerrymander allowance.
  // The card grants a move "immediately", outside the run of play, and for
  // everyone except the active player there is no turn for it to come out of.
  const r = Board.gerrymander(game.board, player.id, rightsZoneId, from, to, {
    allowMajority: round.options.allowMajority,
    blockVolatileDestination: round.options.blockVolatileDestination,
  })
  if (r.error) return { error: r.error }
  ...
  next = addLog(next, {
    type: 'gerrymander',
    message: `${player.name} Gerrymandered a voter (${round.cardName}).`,
  })
  return { game: next, note: `moved a voter using ${rightsZoneId}` }
}

/** One player's pick during A Trip To Goalpara. */
function applyRoundCashOut(game, round, player, { openIndex, allocation = null }) {
  const cardId = game.market.open[openIndex]
  if (!cardId) return { error: 'Pick one of the open Voter Cards' }

  const card = Voter.getVoterCard(cardId)
  if (!card) return { error: `Unknown Voter Card ${cardId}` }

  // "They will receive the resources denoted on these cards." A card's pips are
  // its cost, and most of them are wildcards, so which resources a wildcard pip
  // pays out is the player's call — `allocation` carries it. Without one we
  // spread the wildcards round-robin so the round can never stall on a missing
  // choice.
  const want = cashOutValue(card, allocation)
  if (want.error) return { error: want.error }

  const taken = R.takeFromReserve(player.pool, game.reserve, want.gains)
  let next = { ...game, reserve: taken.reserve }
  next = updatePlayer(next, player.id, { pool: taken.pool })

  // The card leaves the row, and the row is NOT refilled — "New Voter Cards will
  // only open after all 3 cards have been discarded." That is the whole point:
  // each player picks from a shorter row than the last.
  next = {
    ...next,
    market: {
      ...next.market,
      open: next.market.open.filter((_, i) => i !== openIndex),
      discard: [...next.market.discard, cardId],
    },
  }

  next = addLog(next, {
    type: 'resolution',
    message: `${player.name} cashed out ${card.name || 'a Voter Card'}${
      taken.shortfall ? ' (the Reserve fell short)' : ''
    }.`,
  })
  return { game: next, note: `cashed out ${cardId}` }
}

/**
 * What a Voter Card pays when it is cashed out rather than bought.
 *
 * Specific pips pay their own type. Wildcard pips pay whatever the player asks
 * for, and round-robin across the resource types when they do not ask.
 */
export function cashOutValue(card, allocation = null) {
  const cost = R.normalizeCost(card.cost || {})
  const wild = cost.any || 0
  const gains = R.emptyPool()
  for (const [k, v] of Object.entries(cost)) {
    if (k !== 'any' && gains[k] !== undefined) gains[k] += v
  }

  if (allocation) {
    const chosen = { ...R.emptyPool(), ...allocation }
    if (R.poolTotal(chosen) !== wild) {
      return { error: `Choose exactly ${wild} resource(s) for the wildcard pips` }
    }
    for (const k of Object.keys(gains)) gains[k] += chosen[k]
  } else {
    for (let i = 0; i < wild; i++) gains[RESOURCE_IDS[i % RESOURCE_IDS.length]] += 1
  }
  return { gains }
}

/** Take a finished round down, and do whatever it deferred to the end. */
function closeRound(game, rng) {
  const round = game.round
  let next = { ...game, round: null }

  if (round.kind === 'cashOutVoter' && round.options.holdRefill) {
    // Held back until now on purpose, so nobody picks from a row that quietly
    // refilled behind them.
    next = { ...next, market: Voter.refillMarket(next.market, rng) }
  }

  return addLog(next, {
    type: 'resolution',
    message: `${round.cardName || 'The card'} has been round the table.`,
  })
}
 ...
 */
function applySideEffects(game, result, cardId = null) {
  let next = game
  if (result.auction) {
  ...
  if (result.placeJumla) {
    // Jumla does not resolve and leave — it joins the player's Ideology stack
    // and stays. See lib/shasn/jumla.js.
    next = {
      ...next,
      players: Jumla.place(next.players, result.placeJumla.playerId, result.placeJumla.ideologue),
    }
  }
  if (result.round) {
    // Submerged and A Trip To Goalpara stop the game and walk it round the
    // table. The queue is built here rather than in applyEffect because it
    // needs the seating, which is game state.
    const { kind, from, include = true, count = null, options = {} } = result.round
    const queue = Rounds.seatsFrom(next.players, from, { include, count })
    const card = Cards.getHeadlineCard(cardId) || Cards.getConspiracyCard(cardId)
    next = {
      ...next,
...
export function endTurn(game, rng) {
  ...
  if (game.round) {
    return { error: `${game.round.cardName || 'A card'} is still going round the table` }
  }
