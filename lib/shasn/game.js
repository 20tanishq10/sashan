...
export function redrawIdeology(game, rng, allocation = null) {
  ...
  let next = { ...game, ideologyDeck: r.deck, pendingIdeologyCard: r.cardId, reserve: r.reserve }
...
/** Answer the pending Ideology Card, taking the chosen Ideologue's payout. */
export function answerIdeology(game, ideologue) {
  if (game.turnPhase !== TURN_PHASES.IDEOLOGY) return { error: 'Not awaiting an Ideology answer' }
  const player = activePlayer(game)
...
/** Record the outcome of a card the table resolved by hand. */
export function resolveManually(game, { note }) {
  if (!game.awaitingResolution) return { error: 'Nothing is awaiting resolution' }
  const { kind, cardId } = game.awaitingResolution

  let next = { ...game, awaitingResolution: null }
  if (kind === 'conspiracy') {
    const player = game.players.find((p) => p.id === game.awaitingResolution.playerId)
    next = updatePlayer(next, player.id, {
      conspiracyCards: player.conspiracyCards.filter((c) => c !== cardId),
    })
  ...
  return {
    game: addLog(next, { type: 'resolution', message: note || 'Resolved at the table.' }),
  }
}
...
function finishGame(game) {
  const table = Board.standings(
    game.board,
    game.players.map((p) => ({ id: p.id, nickname: p.name }))
  )
  const winner = table[0]
  return addLog(
    { ...game, phase: GAME_PHASES.FINISHED, turnPhase: null, pendingIdeologyCard: null },
    {
      type: 'system',
      message: `Election over. ${winner.nickname} wins with ${winner.score} majority voters.`,
    }
  )
}
