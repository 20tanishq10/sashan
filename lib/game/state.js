...
export function initBoardState(playerIds) {
  return {
    ...
    turnOrder: [...playerIds],
    // Alliance state
    pendingAlliances: [],   // { id, proposerId, targetId, proposerBloc, targetBloc, round }
    resolvedAlliances: [],  // { ...pact, choices: { [id]: 'honor'|'betray' }, resolvedAt }
    // Event tracking
    usedEventIds: [],
    lastEventCard: null,    // { id, name, description } of the most recently fired event
    // Scoring checkpoint snapshots
    checkpointSnapshots: {}, // { [round]: [ { playerId, nickname, total } ] }
    log: [{
      type: 'system',
 ...
 * deltaAll is a per-player adjustment (positive or negative) to a bloc.
 */
...
export function applyAction(gameState, playerStates, playerId, action, playerNickname) {
  ...
  if (action.type === 'play_card') {
    ...
    if (card.cardType === 'scandal') {
      ...
      // Standard policy card — positive effects on self
